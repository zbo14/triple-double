...
class Server extends EventEmitter {
  ...
  static validateBundle (bundle) {
    ...
    const pubSignPreKey = Buffer.from(bundle.pubSignPreKey, 'base64')
  ...
  async handleGetBundleRequest (req, resp) {
    const id = decodeURIComponent(req.url.split('/bundle/')[1])
  ...
  async handleGetMessageRequest (req, resp) {
    const sid = decodeURIComponent(req.url.split('/message/')[1])
